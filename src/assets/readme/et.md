...
UI translations asuvad praegu failis:

```text
src/app/app.translates.ts
```

Keele metaandmed asuvad siin:
...
Translations lisamisel või uuendamisel:

- hoia language codes kooskõlas `LanguageCode`-iga
- uuenda `LANGUAGES`, kui lisad või nimetad ümber toetatud keele
- salvesta translation text ja language labels päris UTF-8 märkidena, mitte escaped või uuesti kodeeritud mojibake kujul
- hoia English source strings stabiilsena, kui sa ei plaani uuendada iga translation entry't
