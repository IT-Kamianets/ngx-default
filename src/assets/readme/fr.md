...
Les UI translations se trouvent actuellement dans :

```text
src/app/app.translates.ts
```

Les métadonnées de langue se trouvent dans :
...
Lors de l'ajout ou de la mise à jour de translations :

- gardez les language codes alignés avec `LanguageCode`
- mettez à jour `LANGUAGES` lorsque vous ajoutez ou renommez une langue prise en charge
- stockez le translation text et les language labels comme de vrais caractères UTF-8, non escaped ou mojibake réencodé
- gardez les English source strings stables sauf si vous comptez mettre à jour chaque translation entry
