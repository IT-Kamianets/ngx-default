...
Az UI translations jelenleg itt találhatók:

```text
src/app/app.translates.ts
```

A nyelvi metaadatok itt találhatók:
...
Translations hozzáadásakor vagy frissítésekor:

- tartsd a language codes értékeket összhangban a `LanguageCode` típussal
- frissítsd a `LANGUAGES` értéket, amikor új támogatott nyelvet adsz hozzá vagy átnevezel egyet
- a translation text és language labels valódi UTF-8 karakterekként legyen tárolva, ne escaped vagy újrakódolt mojibake formában
- az English source strings maradjanak stabilak, hacsak nem akarod az összes translation entry-t frissíteni
