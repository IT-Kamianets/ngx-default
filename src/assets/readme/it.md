# Template Angular per Landing Page (SSR + Prerender)
...
# Eseguire il server SSR (opzionale)
...
Le traduzioni UI attualmente si trovano in:

```text
src/app/app.translates.ts
```

I metadati delle lingue si trovano in:
...
Quando aggiungi o aggiorni le traduzioni:

- mantieni i codici lingua allineati con `LanguageCode`
- aggiorna `LANGUAGES` quando aggiungi o rinomini una lingua supportata
- salva il testo tradotto e le etichette lingua come veri caratteri UTF-8, non escape
  o mojibake ricodificato
- mantieni stabili le stringhe sorgente inglesi, a meno che tu non voglia aggiornare tutte le traduzioni
...
## Struttura feature per moduli collegati al back-end
...
# Crea un nuovo progetto da questo template
...
## Inizializza il tuo repository git
