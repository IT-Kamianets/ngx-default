...
UI translations staan momenteel in:

```text
src/app/app.translates.ts
```

Taalmetadata staat in:
...
Bij het toevoegen of bijwerken van translations:

- houd language codes in lijn met `LanguageCode`
- werk `LANGUAGES` bij wanneer je een ondersteunde taal toevoegt of hernoemt
- sla translation text en language labels op als echte UTF-8-tekens, niet escaped of opnieuw gecodeerde mojibake
- houd English source strings stabiel tenzij je van plan bent elke translation entry bij te werken
