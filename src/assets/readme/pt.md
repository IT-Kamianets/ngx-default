# Modelo Angular para Landing Pages (SSR + Prerender)
...
# Executando o servidor SSR (opcional)
...
As traduções da UI atualmente ficam em:

```text
src/app/app.translates.ts
```

Os metadados de idioma ficam em:
...
- mantenha os códigos de idioma alinhados com `LanguageCode`
- atualize `LANGUAGES` ao adicionar ou renomear um idioma suportado
- armazene o texto das traduções e os nomes dos idiomas como caracteres UTF-8 reais, não escapados nem com codificação corrompida
- mantenha as strings-fonte em inglês estáveis, a menos que você pretenda atualizar todas as entradas de tradução
...
## Estrutura de feature para módulos conectados ao back-end
...
# Criar um novo projeto a partir deste modelo
...
## Inicializar o seu próprio repositório git
